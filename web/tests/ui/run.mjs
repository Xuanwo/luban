...
import { runInboxFilter } from './scenarios/inbox-filter.mjs'
...
async function main() {
	...
	if (explicitBaseUrl) {
		...
		const requestedPort = process.env.LUBAN_AGENT_BROWSER_PORT == null ? null : Number.parseInt(process.env.LUBAN_AGENT_BROWSER_PORT, 10)
	...
	try {
		...
		await runInboxFilter({ page, baseUrl })
